...
class PriceTrackingService:
    ...
    async def get_deal_price_history(
        ...
        """Get price history for a deal directly.
        
        Args:
            deal_id: Deal ID to get price history for
            user_id: User ID requesting the history
            days: Number of days of history to return
            
        Returns:
            List of price points for the deal
            
        Raises:
            PriceTrackingError: If retrieval fails
        ...
        try:
            ...
            # Query for price points within time range for this deal
            query = (
                select(PricePoint)
                .where(
                    PricePoint.deal_id == deal_id,
                    PricePoint.timestamp >= start_date,
                    PricePoint.timestamp <= end_date
                )
                .order_by(PricePoint.timestamp.desc())
            )
            ...
            if not points:
                # If no points found within range, get the most recent 5 regardless of date
                query = (
                    select(PricePoint)
                ...
                if not points:
                    # Still no points, return empty list
                    logger.warning(f"No price points found for deal {deal_id}")
                    return []
            
            return [PricePointResponse.model_validate(p) for p in points]
            
        except Exception as e:
            logger.error(f"Error getting deal price history: {str(e)}")
            raise PriceTrackingError(f"Failed to get deal price history: {str(e)}")
