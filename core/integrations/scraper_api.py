...
class ScraperAPIService:
    ...
    def __init__(
        ...
        self.timeouts = {
            ...
            MarketType.GOOGLE_SHOPPING.value.lower(): aiohttp.ClientTimeout(total=60),  # Increased to 60 seconds for Google Shopping
            MarketType.WALMART.value.lower(): aiohttp.ClientTimeout(total=25),  # 25 seconds for Walmart
            MarketType.EBAY.value.lower(): aiohttp.ClientTimeout(total=30),  # 30 seconds for eBay
            "default": aiohttp.ClientTimeout(total=20)  # Default timeout of 20 seconds
        }
        
        # Configure limits from settings
        self.requests_per_second = getattr(settings, "SCRAPER_API_REQUESTS_PER_SECOND", 8)  # Increased from 5 to 8
    ...
    async def _make_request(
        ...
        logger.debug(f"Using timeout of {timeout.total} seconds for market type: {market_type}")
                ...
                try:
                    async with aiohttp.ClientSession(timeout=timeout) as session:
                        ...
                        if response.status == 200:
                            ...
                            logger.warning(f"404 Not Found error for {url} (attempt {attempt + 1}/{retries})")
                            # For structured endpoints, especially Google Shopping, a 404 often means no results
                            if 'google_shopping' in url:
                                logger.info("Google Shopping returned 404, treating as empty results")
                                # Return empty results rather than raising an exception for Google Shopping
                                return {'products': [], 'status': 'empty_results', 'message': 'No products found'}
                            
                            # For other endpoints, try again or eventually raise an error
                            if attempt < retries - 1:
                                wait_time = 2 ** attempt  # Exponential backoff: 1, 2, 4, 8 seconds...
                                logger.info(f"Waiting {wait_time} seconds before retry for 404 error")
                            ...
                            logger.error(f"Server error (status 500) on attempt {attempt + 1}/{retries}")
                            if attempt < retries - 1:
                                wait_time = 2 ** attempt  # Exponential backoff: 1, 2, 4 seconds...
                                logger.info(f"Waiting {wait_time} seconds before retry for 500 error")
                                await asyncio.sleep(wait_time)
                                continue
                            ...
                            if attempt < retries - 1:
                                wait_time = 2 ** attempt  # Exponential backoff: 1, 2, 4 seconds...
                                logger.info(f"Waiting {wait_time} seconds before retry for status {response.status}")
                                await asyncio.sleep(wait_time)
                                continue
                            ...
                            raise MarketIntegrationError(
                                ...
                                details={'status': response.status, 'response': response_text[:500]}
                            )

                except asyncio.TimeoutError:
                    logger.warning(f"Request timed out (attempt {attempt + 1}/{retries})")
                    # Return empty results instead of retrying
                    return {'shopping_results': [], 'status': 'timeout', 'message': 'Request timed out'}

                except Exception as e:
                    logger.error(f"Error making request: {str(e)}")
                    # Return empty results instead of retrying
                    return {'shopping_results': [], 'status': 'error', 'message': f'Request failed: {str(e)}'}
    ...
    def _record_request_completion(self, market_type='default'):
        ...
        for market in self._market_request_times:
            self._market_request_times[market] = [t for t in self._market_request_times[market] if t >= cutoff_time]
    ...
    async def _apply_rate_limiting(self, market_type='default'):
        ...
        async with self._rate_limit_lock:
            ...
            requests_last_second = len([t for t in self._request_times if t >= current_time - 1])
            ...
            if requests_last_second >= self.requests_per_second:
                ...
                logger.debug(f"Global rate limiting: waiting {wait_time:.2f} seconds before next request")
                await asyncio.sleep(wait_time)
            ...
            if market_type in self._market_request_times:
                ...
                requests_last_second = len([t for t in market_requests if t >= current_time - 1])
                ...
                if requests_last_second >= market_limit:
                    ...
                    logger.debug(f"Market-specific rate limiting for {market_type}: waiting {wait_time:.2f} seconds")
                    await asyncio.sleep(wait_time)
            ...
            for idx, product in enumerate(products):
                try:
                    ...
                    try:
                        ...
                        for price_field in ['price', 'current_price', 'sale_price', 'offer_price', 'min_price']:
                            if price_field in product and product[price_field] is not None:
                                try:
                                    ...
                                    if price_str.lower() in ['unavailable', 'n/a', 'sold out', '', 'null', 'none']:
                                        continue
                                    ...
                                    if not price_str or price_str.isspace() or '.' == price_str:
                                        continue
                                        
                                    price = float(price_str)
                                    
                                    # Validate price is reasonable
                                    if price <= 0 or price > 1000000:  # 1 million max price
                                        logger.debug(f"Extracted price ${price} is outside reasonable range for product {product_id}")
                                        price = None
                                        continue
                        ...
                        if price is None:
                            ...
                            critical_markets = ['amazon', 'walmart', 'bestbuy', 'target']
                            market_str = product.get('source', '').lower() if 'source' in product else ''
                            if any(market in market_str for market in critical_markets):
                                ...
                                logger.debug(f"Could not extract valid price for product {product_id}")
                            continue
                    except Exception as e:
    ...
    async def _init_redis_client(self):
        ...
        if hasattr(self, '_last_redis_init_attempt') and time.time() - self._last_redis_init_attempt < 30:
        ...
        try:
            ...
            try:
                ...
                if pending:
                    ...
                    for task in pending:
                        task.cancel()
                    logger.debug("Redis ping test failed: timeout after 2 seconds, continuing without Redis")
                    self.redis_client = None
                    return
                ...
                if ping_task in done and ping_task.result():
                    ...
                    if not hasattr(self, '_last_redis_success_log') or time.time() - self._last_redis_success_log > 300:
                        logger.info("Redis client initialized successfully for ScraperAPIService")
                        self._last_redis_success_log = time.time()
                else:
                    logger.debug("Redis ping test failed: ping returned false, continuing without Redis")
                    self.redis_client = None
            except asyncio.TimeoutError:
                logger.debug("Redis ping test failed: TimeoutError, continuing without Redis")
                self.redis_client = None
            except ConnectionRefusedError:
                logger.debug("Redis ping test failed: ConnectionRefusedError, continuing without Redis")
                self.redis_client = None
            except Exception as ping_error:
                logger.debug(f"Redis ping test failed: {str(ping_error)}, continuing without Redis")
                self.redis_client = None
        
        except Exception as e:
            logger.debug(f"Failed to initialize Redis client: {str(e)}, continuing without Redis")
            self.redis_client = None
    ...
    async def _record_market_metrics(
        ...
        try:
            ...
            if self.db is None:
                logger.debug("Cannot record market metrics: no database session provided")
                return
            ...
            if self.metrics_service is None:
                try:
                    ...
                    logger.debug(f"Initialized market metrics service for {market_type.value}")
                except Exception as e:
                    logger.warning(f"Failed to initialize market metrics service: {str(e)}")
                    return
            ...
            if result:
                logger.debug(f"Successfully recorded market metrics for {market_type.value}")
            else:
                logger.warning(f"Failed to record market metrics for {market_type.value}")
                
        except Exception as e:
            # Don't let metrics recording failures disrupt the main flow
            logger.warning(f"Error recording market metrics for {market_type.value}: {str(e)}")
    ...
    async def search_google_shopping(
        ...
        if redis_client:
            try:
                ...
                if cached_data:
                    logger.info(f"Using cached Google Shopping results for query: {search_query}")
        ...
        # Update to use the correct structured endpoint format
            ...
            async with aiohttp.ClientSession() as session:
                try:
                    async with session.get(
                        ...
                        response_time = time.time() - start_time
                        logger.debug(f"Response received in {response_time:.2f} seconds with status: {response.status}")
                        
                        if response.status == 200:
                            data = await response.json()
                            
                            logger.debug(f"Raw Google Shopping response keys: {list(data.keys() if isinstance(data, dict) else [])}")
                            
                            # Extract shopping results
                            if 'shopping_results' in data:
                                products = data.get('shopping_results', [])
                                logger.info(f"Found {len(products)} products in shopping_results")
                                
                                # Normalize the products
                                normalized_products = await self._normalize_google_shopping_products(products, search_query)
                                
                                # Limit results if needed
                                if results_limit and normalized_products and len(normalized_products) > results_limit:
                                    normalized_products = normalized_products[:results_limit]
                                ...
                                if redis_client and normalized_products:
                                    try:
                                        ...
                                        logger.debug(f"Cached Google Shopping results for query: {search_query}")
                                    except Exception as cache_error:
                                        logger.warning(f"Error caching results: {str(cache_error)}")
                                ...
                                logger.info(f"Found {len(products)} products in products key")
                                ...
                                logger.warning(f"Unexpected response structure: {list(data.keys()) if isinstance(data, dict) else type(data)}")
                            ...
                            logger.info(f"Google Shopping returned 404 for '{search_query}', treating as empty results")
                            return []
                        else:
    ...
    async def _normalize_google_shopping_products(
        ...
        for i, product in enumerate(products):
            try:
                ...
                if not product_id or product_id == 'null' or product_id == 'undefined':
                    logger.warning(f"Invalid product ID for item {i}, generating new UUID")
                ...
                if not title or title == 'null' or title == 'undefined':
                    logger.warning(f"Missing or invalid title for product {product_id}, skipping")
                    continue
                ...
                if not price_str:
                    for price_field in ['current_price', 'sale_price', 'discounted_price', 'offered_price']:
                        if price_field in product and product[price_field]:
                            price_str = product[price_field]
                            logger.debug(f"Using alternative price field: {price_field}")
                            break
                ...
                try:
                    if isinstance(price_str, (int, float)):
                        ...
                        if isinstance(price_value, (int, float)):
                            ...
                            clean_price = price_value.replace('$', '').replace('£', '').replace('€', '').replace(',', '').strip()
                            import re
                    ...
                    logger.warning(f"Failed to parse price '{price_str}' for product {product_id}: {e}")
                
                # Try to fallback to integer price fields if string parsing failed
                if price is None:
                    for alt_price_field in ['price_cents', 'price_amount', 'cents', 'amount']:
                        if alt_price_field in product and isinstance(product[alt_price_field], (int, float)):
                            ...
                            if cents_value > 100:
                                ...
                                price = cents_value
                            logger.info(f"Using alternative price field {alt_price_field}: {price}")
                            break
                
                # Last resort fallbacks for specific API responses
                if price is None and 'pricing' in product and isinstance(product['pricing'], dict):
                    pricing_obj = product['pricing']
                    for price_key in ['current_price', 'sale_price', 'regular_price', 'price']:
                        if price_key in pricing_obj and pricing_obj[price_key] is not None:
                            try:
                                if isinstance(pricing_obj[price_key], (int, float)):
                                    price = float(pricing_obj[price_key])
                                elif isinstance(pricing_obj[price_key], str) and pricing_obj[price_key].strip():
                                    clean_price = pricing_obj[price_key].replace('$', '').replace('£', '').replace('€', '').replace(',', '').strip()
                                    import re
                                    digits = re.findall(r'[\d.]+', clean_price)
                                    if digits:
                                        price = float(digits[0])
                                        logger.info(f"Using nested pricing.{price_key}: {price}")
                                        break
                            except (ValueError, TypeError):
                                pass
                ...
                if price is None:
                    logger.warning(f"Could not extract valid price for product {product_id}, skipping")
                    continue
                ...
                for orig_price_field in ['original_price', 'was_price', 'previous_price', 'list_price', 'msrp', 'regular_price']:
                    if orig_price_field in product and product[orig_price_field]:
                ...
                try:
                    if isinstance(original_price_str, (int, float)):
                    ...
                    elif isinstance(original_price_str, dict) and 'value' in original_price_str:
                        ...
                        clean_price = original_price_str.replace('$', '').replace('£', '').replace('€', '').replace(',', '').strip()
                        ...
                        if digits:
                            ...
                            if original_price and original_price <= price:
                                logger.debug(f"Original price {original_price} not higher than current price {price}, ignoring")
                                original_price = None
                except (ValueError, TypeError) as e:
                ...
                # Extract description
                description = ""
                for desc_field in ['description', 'snippet', 'summary', 'details', 'overview']:
                    if desc_field in product and product[desc_field]:
                ...
                if 'merchant' in product:
                    if isinstance(product['merchant'], dict) and 'name' in product['merchant']:
        ...
        logger.info(f"Successfully normalized {len(normalized_products)} Google Shopping products")
        return normalized_products
    ...
    async def get_google_shopping_product(
        ...
        try:
            ...
            description = ""
            for field in ['description', 'product_description', 'about', 'overview', 'details']:
            ...
            normalized_product = {
                ...
                'market_type': 'google_shopping',
                'merchant': product.get('merchant', {}).get('name', product.get('source', 'Unknown')),
                'rating': float(product.get('rating', {}).get('value', 0)),
                'review_count': int(product.get('rating', {}).get('count', 0)),
                'availability': product.get('availability', 'Unknown'),
                'features': product.get('features', []),
                'is_available': 'out of stock' not in product.get('availability', '').lower(),
                'metadata': {
                    'source': 'google_shopping',
                    'timestamp': datetime.utcnow().isoformat(),
                    'raw_fields': list(product.keys())
    ...
    # Add method alias for market_factory.py compatibility
    async def search_walmart(
        ...
        """Alias for search_walmart_products to maintain compatibility with market_factory."""
        logger.debug(f"Using search_walmart alias for query: '{query}'")
        return await self.search_walmart_products(
            query=query,
            page=page,
            cache_ttl=cache_ttl,
            limit=limit
