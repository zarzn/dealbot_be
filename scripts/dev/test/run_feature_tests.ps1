...
# AI Agentic Deals System - Feature Tests Runner
Write-Host "🚀 Starting AI Agentic Deals System Feature Tests" -ForegroundColor Cyan
...
# Main execution
try {
    Write-Host "`n========== Running Feature Tests ==========" -ForegroundColor Cyan
    ...
    $tempScriptPath = "$env:TEMP\run_feature_tests.py"
...
print(f"Using marker: feature")

exit_code = pytest.main([
    test_path, 
    '-v', 
    '-m', 'feature', 
    '--html='+os.path.join(results_dir, 'feature_report.html'), 
    '--self-contained-html',
...
"@ | Out-File -FilePath $tempScriptPath -Encoding utf8
    ...
    try {
        ...
        Write-Host "Running feature tests..." -ForegroundColor Cyan
        
        # First, remove any existing report to ensure a fresh one is created
        $reportFile = "$RESULTS_DIR\feature_report.html"
    ...
    $summaryFile = "$RESULTS_DIR\feature_test_summary.txt"
    "# Feature Test Results Summary" | Out-File -FilePath $summaryFile -Force
    ...
    if ($exitCode -eq 0) {
        Write-Host "Feature tests completed successfully" -ForegroundColor Green
        "✅ Feature tests: PASSED" | Out-File -FilePath $summaryFile -Append
    } else {
        Write-Host "Feature tests had failures" -ForegroundColor Yellow
        "❌ Feature tests: FAILED or had issues" | Out-File -FilePath $summaryFile -Append
    }
    ...
    $reportFile = "$RESULTS_DIR\feature_report.html"
    if (Test-Path $reportFile) {
        "- feature_report.html" | Out-File -FilePath $summaryFile -Append
    } else {
    ...
    Write-Host "$RESULTS_DIR\feature_report.html" -ForegroundColor Yellow
