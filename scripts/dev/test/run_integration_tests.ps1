...
# AI Agentic Deals System - Integration Tests Runner
Write-Host "🚀 Starting AI Agentic Deals System Integration Tests" -ForegroundColor Cyan
...
try {
    ...
    $tempScriptPath = "$env:TEMP\run_integration_tests.py"
...
print(f"Using marker: integration")

exit_code = pytest.main([
    test_path, 
    '-v', 
    '-m', 'integration', 
    '--html='+os.path.join(results_dir, 'integration_report.html'), 
    '--self-contained-html',
...
"@ | Out-File -FilePath $tempScriptPath -Encoding utf8
    ...
    $summaryFile = "$RESULTS_DIR\integration_test_summary.txt"
    "# Integration Test Results Summary" | Out-File -FilePath $summaryFile -Force
    ...
    if ($exitCode -eq 0) {
        ...
        "✅ Integration tests: PASSED" | Out-File -FilePath $summaryFile -Append
    } else {
        Write-Host "Integration tests had failures" -ForegroundColor Yellow
        "❌ Integration tests: FAILED or had issues" | Out-File -FilePath $summaryFile -Append
    }
    ...
    if (Test-Path $reportFile) {
        "- integration_report.html" | Out-File -FilePath $summaryFile -Append
    } else {
    ...
    Write-Host "$RESULTS_DIR\integration_report.html" -ForegroundColor Yellow
