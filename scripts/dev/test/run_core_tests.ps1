...
# AI Agentic Deals System - Core Tests Runner
Write-Host "🚀 Starting AI Agentic Deals System Core Tests" -ForegroundColor Cyan
...
try {
    ...
    $tempScriptPath = "$env:TEMP\run_core_tests.py"
...
"@ | Out-File -FilePath $tempScriptPath -Encoding utf8
    ...
    $summaryFile = "$RESULTS_DIR\core_test_summary.txt"
    "# Core Test Results Summary" | Out-File -FilePath $summaryFile -Force
    ...
    if ($exitCode -eq 0) {
        ...
        "✅ Core tests: PASSED" | Out-File -FilePath $summaryFile -Append
    } else {
        Write-Host "Core tests had failures" -ForegroundColor Yellow
        "❌ Core tests: FAILED or had issues" | Out-File -FilePath $summaryFile -Append
    }
    ...
    if (Test-Path $reportFile) {
        "- core_report.html" | Out-File -FilePath $summaryFile -Append
    } else {
    ...
    Write-Host "$RESULTS_DIR\core_report.html" -ForegroundColor Yellow
