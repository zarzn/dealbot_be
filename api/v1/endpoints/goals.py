...
async def get_shared_goals(
    ...
    try:
        shared_goals = await goal_service.get_shared_goals(
            user_id=current_user["id"]
        )
        return shared_goals
    except Exception as e:
...
async def get_goal_shared_users(
    ...
    """Get users with whom the goal is shared"""
    try:
        shared_users = await goal_service.get_shared_users(
            goal_id=goal_id,
            user_id=current_user["id"]
        )
        return shared_users
...
async def remove_goal_share(
    ...
    """Remove goal sharing for a specific user"""
    try:
        ...
        return {"message": "Goal sharing removed successfully"}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def get_goals_progress(
    ...
    """Get progress analytics for all user goals"""
    try:
        # Convert time range to datetime
        now = datetime.utcnow()
        ranges = {
