...
async def get_market_price_history(
    ...
    try:
        ...
        history = await analytics_service.get_market_price_history(
            market_id=market_id,
...
async def get_market_availability(
    ...
    try:
        ...
        availability = await analytics_service.get_market_availability(
            market_id=market_id,
            category=category
        )
...
async def get_market_performance(
    ...
    try:
        ...
        performance = await analytics_service.get_market_performance(
            market_id=market_id,
            start_date=start_date
        )
...
async def search_market(
    ...
    try:
        # Validate tokens before searching
        await token_service.validate_operation(current_user.id, "market_search")
        
        search_result = await market_search_service.search_products(
            query=query,
            market_types=[market_id],
            category=category,
            min_price=min_price,
            max_price=max_price,
            limit=limit
